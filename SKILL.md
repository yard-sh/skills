...
- `migrations.dir` - migrations directory relative to the working directory (default `.yard/migrations`). Flat numbered `.sql` files (`0001_init.sql`, `0002_add_column.sql`, ...), pushed with the release and applied to each scope's database in filename order before any new services are deployed; the `_yard_migrations` table in the database records applied files by filename, so each file runs once. The directory must not sit inside a service directory.
