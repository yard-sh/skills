...
1. **Simulated sales are never in the seller's books.** Earnings, payouts, `yard transactions list` and `yard customers list` read the project's own commerce only. A sandbox's customers, transactions, subscriptions and license keys appear on that sandbox's pages in the dashboard, and nowhere in the CLI.
...
- `migrations.dir` - migrations directory relative to the working directory (default `.yard/migrations`). Flat numbered `.sql` files (`0001_init.sql`, `0002_add_column.sql`, ...), pushed with the release and applied to each scope's database in filename order before new service code goes live; the `_yard_migrations` table in the database records applied files by filename, so each file runs once. The directory must not sit inside a service directory.
