...
- `landing_page.type` - which page the release serves: `custom` serves the files in `landing_page.dir`, `default` serves the default page you edit in the dashboard (files in the directory are still uploaded with the release, just not served). A `landing_page` block without `type` means `custom`. Omitting the whole block leaves the page type and files as they already are on the release you are building from; a brand new project starts on the default page. A `custom` block whose directory holds no files is an error on `yard push`, on GitHub release sync, and when creating a project from a template. Custom pages are plan-gated: declaring one without the feature fails with `upgrade_required`.
...
- **Yard Auth** - Buyer sign-in for a project: the edge signs buyers into hosted services and stamps identity and entitlement headers, and any external app can use the project as a standard OpenID Connect provider (Basic and Pro, `yard_auth` permission)
...
| REST API (integration endpoints for license validation, releases, subscriptions, Yard Auth for external apps) | [references/api-reference.md](references/api-reference.md)                 |
...
| Service & database - runtime contract, `yard service` workflow, Yard Auth headers and session endpoints, database | [references/service-and-database.md](references/service-and-database.md)   |
